...
namespace Elys {
	...
	static std::string Rigidbody2DBodyTypeToString(Rigidbody2DComponent::BodyType bodyType)
	{
		...
		ELYS_CORE_ASSERT(false, "Unknown body type");
		return {};
	}
	...
	{
		ELYS_CORE_ASSERT(entity.HasComponent<IDComponent>());
	...
	bool SceneSerializer::Deserialize(const std::string& filepath)
	{
		...
		ELYS_CORE_TRACE("Deserializing scene '{0}'", sceneName);
		...
		if (entities)
		{
			for (auto entity : entities)
			{
				...
				ELYS_CORE_TRACE("Deserialized entity with ID = {0}, name = {1}", uuid, name);

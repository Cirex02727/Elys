...
#include "Elys/Renderer/UniformBuffer.h"
...
namespace Elys {
	...
	void Renderer2D::DrawQuad(const glm::vec2& position, const glm::vec2& size, const Ref<SubTexture2D>& subtexture, const glm::vec2& tilingFactor, const glm::vec4& tintColor)
	{
		DrawQuad({ position.x, position.y, 0.0f }, size, subtexture, tilingFactor, tintColor);
	}

	void Renderer2D::DrawQuad(const glm::vec3& position, const glm::vec2& size, const Ref<SubTexture2D>& subtexture, const glm::vec2& tilingFactor, const glm::vec4& tintColor)
	{
		ELYS_PROFILE_FUNCTION();
	...
	void Renderer2D::DrawQuad(QuadVertex* quads)
	{
		if (s_Data.QuadIndexCount >= Renderer2DData::MaxIndices)
			NextBatch();

		for (int i = 0; i < 4; i++) {
			s_Data.QuadVertexBufferPtr->Position = quads->Position;
			s_Data.QuadVertexBufferPtr->Color = quads->Color;
			s_Data.QuadVertexBufferPtr->TexCoord = quads->TexCoord;
			s_Data.QuadVertexBufferPtr->TexIndex = quads->TexIndex;
			s_Data.QuadVertexBufferPtr->TilingFactor = quads->TilingFactor;
			s_Data.QuadVertexBufferPtr++;
	...
	void Renderer2D::DrawRotatedQuad(const glm::vec2& position, const glm::vec2& size, float rotation, const Ref<SubTexture2D>& subtexture, const glm::vec2& tilingFactor, const glm::vec4& tintColor)
	{
		DrawRotatedQuad({ position.x, position.y, 0.0f }, size, rotation, subtexture, tilingFactor, tintColor);
	}

	void Renderer2D::DrawRotatedQuad(const glm::vec3& position, const glm::vec2& size, float rotation, const Ref<SubTexture2D>& subtexture, const glm::vec2& tilingFactor, const glm::vec4& tintColor)
	{
		ELYS_PROFILE_FUNCTION();

			...
			void BeginSession(const std::string& name, const std::string& filepath = "results.json")
			{
				...
				if (m_CurrentSession) {
					...
					if (Log::GetCoreLogger()) { // Edge case: BeginSession() might be before Log::Init()
						ELYS_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
					}
					InternalEndSession();
				}
				...
				if (m_OutputStream.is_open()) {
					...
					if (Log::GetCoreLogger()) { // Edge case: BeginSession() might be before Log::Init()
						ELYS_CORE_ERROR("Instrumentor could not open results file '{0}'.", filepath);
					}
				}
			}
...
#define ELYS_PROFILE 0
	...
	// could mark the wrong one in your editor!
		...
		#define ELYS_FUNC_SIG __PRETTY_FUNCTION__
	#elif defined(__DMC__) && (__DMC__ >= 0x810)
		#define ELYS_FUNC_SIG __PRETTY_FUNCTION__
	#elif defined(__FUNCSIG__)
		#define ELYS_FUNC_SIG __FUNCSIG__
	#elif (defined(__INTEL_COMPILER) && (__INTEL_COMPILER >= 600)) || (defined(__IBMCPP__) && (__IBMCPP__ >= 500))
		#define ELYS_FUNC_SIG __FUNCTION__
	#elif defined(__BORLANDC__) && (__BORLANDC__ >= 0x550)
		#define ELYS_FUNC_SIG __FUNC__
	#elif defined(__STDC_VERSION__) && (__STDC_VERSION__ >= 199901)
		#define ELYS_FUNC_SIG __func__
	#elif defined(__cplusplus) && (__cplusplus >= 201103)
		#define ELYS_FUNC_SIG __func__
	#else
		#define ELYS_FUNC_SIG "ELYS_FUNC_SIG unknown!"
	#endif
	...
	#define ELYS_PROFILE_END_SESSION()
	#define ELYS_PROFILE_SCOPE(name)
	#define ELYS_PROFILE_FUNCTION()
#endif
